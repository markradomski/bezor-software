...
class DB extends Emitter {
  ...
  listen() {
    admin
      .database()
      .ref(`/${this.path}`)
      .on('child_changed', snap => this.emit('change', snap.val()));
    admin
      .database()
      .ref(`/${this.path}`)
      .on('child_removed', snap => this.emit('remove', snap.val()));
    admin
      .database()
      .ref(`/${this.path}`)
      .on('child_added', snap => this.emit('add', snap.val()));
  }
}
