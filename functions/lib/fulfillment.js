...
class Fulfillment {
  ...
  createInvoice() {
    ...
    const invoice = {
      ...
      payTo: `Bezor Software,
  100 Main St, Adelaide`
    };
    ...
    if (sale.processor === 'stripe') {
      ...
      invoice.billTo = invoice.billToLines
        .filter(s => s && s != '' > 0)
        .join('\n');
    ...
    if (downloadIds.length > 0) {
      for (var id of downloadIds) {
        ...
        invoice.deliverables[id] = {
          ...
          downloadUrl: `https://us-central1-bezor-software.cloudfunctions.net/download?oid=${sale.id}&id=${id}`,
          quantity: item.quantity,
          image: item.image,
          limit: item.limit,
          size: item.size,
