...
const Cart = {
  ...
  save: function() {
    localStorage.setItem('bezorCart', JSON.stringify(this.items));
  },
  load: function() {
    this.items = [];
    var items = JSON.parse(localStorage.getItem('bezorCart'));
  ...
  generateId: function() {
    ...
    const key = shortid(4);
    const id = `BEZOR-${formattedDate}-${key}`;
    return id;
  }
};
